...
class MapBoxAPI(BaseAPI):
    ...
    def get_access_token(self, reload=False):
        ...
        self.access_token = (
            "pk.eyJ1Ijoic3ltYml1bWRldiIsImEiOiJjazBpdXJiemcwM3p6M2JvM" "jA2bDA2cTJwIn0.aon9ZULViFwIj9Flt9vl7Q"
        )
        return self.access_token
    ...
    def get_roads(self, curr_map):
        ...
        for x_delta, y_delta in tile_keys:
            ...
            mapbox_vtile_url = (
                f"{MapBoxAPI.VTILE_BASE_URL}/{curr_map.zoom}/{x}/{y}.vector.pbf"
            ...
            if "road" in curr_vtile:
                for feature in curr_vtile["road"]["features"]:
                    if feature["geometry"]["type"] != "LineString" or feature["properties"]["class"] != "street":
                        continue
                    curr_frame = [
                        tuple(
                            [
                                256.0 * (float(point[0]) / 4096.0),
